...
# ls -1 /cluster/scratch/afeurtey/Kmer_GWAS/3_Kmer_count/*/kmers_with_strand |  \
#   awk 'BEGIN{OFS="\t"} {  split($0, a, "/"); print $0, a[8] }'  \
#   > /cluster/scratch/afeurtey/Kmer_GWAS/4_All_kmers_GWAS/All_GWAS_kmers_list_paths.txt #no_kmers.list
...
./voichek_kmersGWAS/bin/list_kmers_found_in_multiple_samples \
  -l ${working_dir}All_GWAS_kmers_list_paths.txt \
  -o ${working_dir}All_GWAS_kmers_to_use \
  --mac 5 -p 0.2 -k 31

./voichek_kmersGWAS/bin/build_kmers_table \
  -l ${working_dir}All_GWAS_kmers_list_paths.txt \
  -a ${working_dir}All_GWAS_kmers_to_use \
  -k 31 \
  -o ${working_dir}All_GWAS_kmers_table
...
./voichek_kmersGWAS/bin/emma_kinship_kmers \
  -t ${working_dir}All_GWAS_kmers_table \
  -k 31 --maf 0.05 \
  > ${working_dir}All_GWAS_kmers_table.kinship
