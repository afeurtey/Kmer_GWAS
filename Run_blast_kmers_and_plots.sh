...
# Usage : sh Run_blast_kmers_and_plots.sh ./Phenotypes/Phenotype_temperature_merged_lofoutrem.csv /cluster/scratch/afeurtey/Kmer_GWAS/all_PacBio.fa all_PacBio
...
do
    ...
    sh Blast_all_kmers.sh $i $blastdb_path $blastdb_suffix
    ...
    if [ $found -gt 0 ]; then
        python Manhattan_plots_kmer_GWAS.py $i  $blastdb_suffix
    else
        ref_list=$(cut -f 2 /cluster/scratch/afeurtey/Kmer_GWAS/5_GWAS_results/GWAS_output_dir_OptTemperature/kmers/output/phenotype_value.assoc_for_fetch.blast_all_PacBio.tsv | cut -f 1 -d "." | sort | uniq )
        for ref in $ref_list; do
            echo "Working on reference genome " $ref " for condition " $i
            grep $ref ${GWAS_dir}GWAS_output_dir_${i}/kmers/output/phenotype_value.assoc_for_fetch.blast_${blastdb_suffix}.tsv > \
                    ${GWAS_dir}GWAS_output_dir_${i}/kmers/output/phenotype_value.assoc_for_fetch.blast_${ref}.tsv
            python Manhattan_plots_kmer_GWAS.py $i  $ref
        done
