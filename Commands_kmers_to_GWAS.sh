...
# ls -1 /cluster/scratch/afeurtey/Kmer_GWAS/3_Kmer_count/*/kmers_with_strand | tail -n +2 |  \
#   awk 'BEGIN{OFS="\t"} {  split($0, a, "/"); print $0, a[7] }'  \
#   > ${working_dir}kmers_list_paths.txt #no_kmers.list
...
#   -l ${working_dir}kmers_list_paths.txt \
#   -o ${working_dir}kmers_to_use \
#   --mac 5 -p 0.2 -k 31

./voichek_kmersGWAS/bin/build_kmers_table \
  -l ${working_dir}kmers_list_paths.txt \
  -a ${working_dir}kmers_to_use \
  -k 31 \
  -o ${working_dir}kmers_table
...
./voichek_kmersGWAS/bin/emma_kinship_kmers \
  -t ${working_dir}kmers_table \
  -k 31 --maf 0.05 \
  > ${working_dir}kmers_table.kinship
